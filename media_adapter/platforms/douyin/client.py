...
from media_adapter.core.base_crawler import AbstractApiClient
from media_adapter.proxy.proxy_mixin import ProxyRefreshMixin
from media_adapter.utils import utils
from media_adapter.context import request_keyword_var

if TYPE_CHECKING:
    from media_adapter.proxy.proxy_ip_pool import ProxyIpPool
...
class DouYinClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def pong(self, browser_context: BrowserContext) -> bool:
        """
        Check login status by checking localStorage and cookies.
        """
        # Check localStorage
        try:
            local_storage = await self.playwright_page.evaluate("() => window.localStorage")
            if local_storage.get("HasUserLogin", "") == "1":
                utils.logger.info("[DouYinClient.pong] localStorage shows logged in")
                return True
        except Exception as e:
            utils.logger.warning(f"[DouYinClient.pong] Failed to check localStorage: {e}")

        # Check cookies
        try:
            _, cookie_dict = utils.convert_cookies(await browser_context.cookies())
            if cookie_dict.get("LOGIN_STATUS") == "1":
                utils.logger.info("[DouYinClient.pong] Cookie shows logged in")
                return True
        except Exception as e:
            utils.logger.warning(f"[DouYinClient.pong] Failed to check cookies: {e}")

        utils.logger.info("[DouYinClient.pong] Not logged in")
        return False
