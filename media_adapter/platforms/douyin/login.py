...
from media_adapter.core.base_crawler import AbstractLogin
...
class DouYinLogin(AbstractLogin):
    ...
    async def check_login_state_quick(self):
        """Quick check for login state without retries"""
        try:
    ...
    async def login_by_qrcode(self):
        ...
        utils.logger.info("[DouYinLogin.login_by_qrcode] Waiting for QR code scan (60s timeout)...")
        wait_time = 60
        while wait_time > 0:
            if await self.check_login_state_quick():
                utils.logger.info("[DouYinLogin.login_by_qrcode] Login detected successfully")
                return
